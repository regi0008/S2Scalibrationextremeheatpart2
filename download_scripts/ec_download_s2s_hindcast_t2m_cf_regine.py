#!/usr/bin/env python
'''
This program downloads the ECMWF SFC T2m hindcast data (1 cf) using MARS, from the
...
for index_date in range(len(date_array)):
    ...
    server.retrieve({
        ...
        "time": "00:00:00",
        "type": "cf",
		"target": ecmwf_path + "ecmwf_t2m_" + date_str + "_cf.grib",
    })
    print 'done! Downloaded the .cf file.' + '\n'

    os.system('/grib_to_netcdf -M -I method,type,stream,refdate -T -o ' + ecmwf_path + 'ecmwf_t2m_' + date_str + '_cf.nc ' + ecmwf_path + "ecmwf_t2m_" + date_str + "_cf.grib")
